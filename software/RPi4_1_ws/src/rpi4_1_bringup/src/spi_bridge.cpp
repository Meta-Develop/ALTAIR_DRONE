...
// ISM330DHCX 16-byte Packet Format:
// [0-3]: Header AA BB CC DD
// [4-15]: Raw sensor data: GX_L, GX_H, GY_L, GY_H, GZ_L, GZ_H, AX_L, AX_H, AY_L, AY_H, AZ_L, AZ_H
#define PACKET_BYTES 16
#define SENSOR_DATA_BYTES 12  // 6 int16_t values

// ISM330DHCX Sensitivities (matching firmware config)
...
private:
    ...
    void processData(const std::vector<uint8_t>& rx) {
        ...
        if (rx[0] == HEADER[0] && rx[1] == HEADER[1] && rx[2] == HEADER[2] && rx[3] == HEADER[3]) {
            // Parse raw int16_t sensor data (Little Endian from Pico)
            // Layout: GX(2), GY(2), GZ(2), AX(2), AY(2), AZ(2)
            ...
            if (log_cnt++ % 10 == 0) {
                 RCLCPP_INFO(this->get_logger(), "Accel: [%.2f, %.2f, %.2f] m/s^2 | Gyro: [%.2f, %.2f, %.2f] rad/s",
                             accel[0], accel[1], accel[2], gyro[0], gyro[1], gyro[2]);
            }
                        
            // Publish as Float32MultiArray [ax, ay, az, gx, gy, gz]
