#include "telemetry.h"
#include "telemetry.pio.h"
#include "hardware/gpio.h"
#include <string.h>

// CRC8 Table (Poly 0x07) - Standard for BLHeli/KISS telemetry
...
static uint8_t get_crc8(uint8_t *data, uint8_t len) {
    uint8_t crc = 0;
    for (int i = 0; i < len; i++) {
...
float telemetry_read_rpm(uint motor_index) {
    ...
    // At 115200, 1 byte ~ 86us. 10 bytes ~ 1ms.
    // We wait up to 3ms.
    ...
    if (bytes_read < 10) return 0.0f;
    
    // 4. Decode
    // Frame: [Temp] [VoltageH] [VoltageL] [CurrentH] [CurrentL] [ConsumptionH] [ConsumptionL] [RpmH] [RpmL] [CRC]
    // We only care about RPM (Bytes 7, 8) and CRC (Byte 9).
    
    uint8_t crc = get_crc8(buffer, 9);
    if (crc != buffer[9]) return 0.0f; // CRC Error
    
    uint16_t rpm_raw = (buffer[7] << 8) | buffer[8];
    
    // Convert to float (eRPM or mechanical? Usually eRPM).
    // Let's return raw eRPM for now, node B can filter/scale.
    // Or convert to Hz?
    // Task says "RPM".
    return (float)rpm_raw; // eRPM / pole_pairs -> done in Node B usually.
}
