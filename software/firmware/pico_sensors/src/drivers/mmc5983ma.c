...
bool mmc5983_init(spi_inst_t *spi, uint cs_pin) {
    ...
    // Enable continuous measurement mode at 100Hz
    // CTRL0: TM_M = 0 (single shot), we'll use manual trigger
    // CTRL2: CMM_EN = 0 (disable continuous), we'll trigger manually
    // For now, just leave in default state and trigger per-read
...
void mmc5983_trigger_measurement(spi_inst_t *spi, uint cs_pin) {
    // Set TM_M bit in CTRL0 to trigger a single measurement
    mmc5983_write_reg(spi, cs_pin, MMC5983_CTRL0, 0x01);  // TM_M = 1
}
