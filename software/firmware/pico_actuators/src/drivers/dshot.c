#include "dshot.h"
#include "dshot_bidir.pio.h" // Generated by CMake
...
void dshot_init(void) {
    uint offset = pio_add_program(pio_hw, &dshot_bidir_program);
    ...
    for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        if (current_pio == pio1) {
            if (!pio1_loaded) {
                pio1_offset = pio_add_program(pio1, &dshot_bidir_program);
                pio1_loaded = true;
            }
        ...
        uint sm = (i < 4) ? i : (i - 4);
        dshot_bidir_program_init(current_pio, sm, current_offset, dshot_pins[i]);
...
void dshot_write_throttle(uint8_t motor_index, float throttle, bool telemetry) {
